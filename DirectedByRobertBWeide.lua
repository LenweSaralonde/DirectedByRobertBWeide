...
function DBRBW.Initialize()
	...
	creditsFrame:SetScript('OnKeyDown', function(self, keyValue)
	...
	creditsFrame:SetScript('OnEvent', function()
...
function DBRBW.ShowCredits()
	...
	for _, credit in pairs(DBRBW.GetCredits()) do
		...
		time = time + delay
		table.insert(timers, C_Timer.NewTimer(time, function()
			creditsText:SetText(row1 .. "\n" .. row2)
		end))
	end

	-- Enqueue music
	table.insert(timers, C_Timer.NewTimer(1 + 3.3 - 2.2, function()
		_, musicHandle = PlaySoundFile('Interface\\AddOns\\DirectedByRobertBWeide\\frolic.mp3', 'Master')
	end))

	-- Enqueue animation end sequence

	table.insert(timers, C_Timer.NewTimer(30, function()
		StopSound(musicHandle, 5000)
		musicHandle = nil
	end))

	table.insert(timers, C_Timer.NewTimer(34, function()
		creditsTextAnimation:Play(true)
	end))

	table.insert(timers, C_Timer.NewTimer(35, function()
		creditsFadeAnimation:Play(true)
	end))

	table.insert(timers, C_Timer.NewTimer(36, function()
		DBRBW.HideCredits()
	end))
...
function DBRBW.HideCredits()
	...
	-- Stop music
	if musicHandle then StopSound(musicHandle, 100) end
	musicHandle = nil
	StopMusic()
